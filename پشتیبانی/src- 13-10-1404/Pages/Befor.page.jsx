...
export default function Beforpage() {
  ...
  useEffect(() => {
    ...
    if (savedRows) {
      ...
      fetch("http://127.0.0.1:5000/befor")
        .then((res) => res.json())
        .then((data) => {
  ...
  //=============== ویرایش قیمت =================
    ...
    const selected = rows.find((r) => r.enableCalc && r.calculate === 2);
    if (!selected) {
      alert("لطفاً یک ردیف قابل ویرایش را انتخاب کنید");
      return;
    }
    setEditPriceRow(selected);
    setPriceInput(selected.unitprice ? formatPrice(selected.unitprice) : "");
  };
